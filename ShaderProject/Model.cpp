...
bool Model::Load(const char* file, float scale, bool flip, DirectX::XMFLOAT3 offset)
{
	MeshPool::Ins()->Load(file, &m_meshes, &m_materials, scale, flip, offset);
	return true;
}
...
MeshPool* MeshPool::m_Ins;

bool MeshPool::Load(const char* file, Model::Meshes* outMesh, Model::Materials* outMaterial, float scale, bool flip, DirectX::XMFLOAT3 offset)
{
	if (m_meshes.count(file) != 0 && m_materials.count(file) != 0)
	{
		*outMesh = m_meshes[file];
		*outMaterial = m_materials[file];
		return 1;
	}
	...
	for (unsigned int i = 0; i < pScene->mNumMeshes; ++i)
	{
		...
		for (unsigned int j = 0; j < vtx.size(); ++j)
		{
			...
			// 値を設定
			vtx[j] = {
				DirectX::XMFLOAT3(pos.x * scale + offset.x, pos.y * scale + offset.y, pos.z * scale + offset.z),
		...
		desc.pVtx = vtx.data();
		desc.vtxSize = sizeof(Model::Vertex);
	...
	for (unsigned int i = 0; i < pScene->mNumMaterials; ++i)
	{
		...
		// マテリアル追加
		m_materials[file].push_back(material);
	}
	*outMesh = m_meshes[file];
	*outMaterial = m_materials[file];
